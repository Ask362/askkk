document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!user || user.role !== 'admin') {
        alert('Доступ запрещен. Требуется учетная запись администратора.');
        ...
        fetch('./users.json')
            ...
            .then(users => {
                ...
                document.querySelectorAll('.delete-user').forEach(button => {
                    button.addEventListener('click', function () {
                        const userId = this.getAttribute('data-id');
                        alert(`Удаление пользователя #${userId} (заглушка)`);
                    });
                });
            })
            .catch(error => {
                console.error('Ошибка загрузки пользователей:', error);
                document.getElementById('users-table').innerHTML = '<tr><td colspan="8">Ошибка загрузки данных.</td></tr>';
            });
    }
    ...
    function loadOrders() {
        ...
        .then(([orders, users, products]) => {
            ...
            orders.forEach(order => {
                const user = users.find(u => u.id === order.userId) || { name: 'Неизвестный' };
                const statusClass = {
        ...
        .catch(error => {
            ...
            document.getElementById('orders-table').innerHTML = '<tr><td colspan="8">Ошибка загрузки данных.</td></tr>';
        });
    }
    ...
    document.getElementById('saveUserBtn').addEventListener('click', function () {
        alert('Сохранение пользователя (заглушка)');
    ...
    document.getElementById('addUserBtn').addEventListener('click', function () {
        alert('Добавление пользователя (заглушка)');
    ...
    document.getElementById('addOrderBtn').addEventListener('click', function () {
        ...
        getOrders().then(orders => {
            if (orders.find(o => o.orderId === newOrder.orderId)) {
                alert('Заказ с таким ID уже существует.');
                return;
            }
